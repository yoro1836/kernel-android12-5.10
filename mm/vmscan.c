...
{
	unsigned long new_flags, old_flags = READ_ONCE(folio_flags(folio));

	VM_WARN_ON_ONCE(gen >= MAX_NR_GENS);
	VM_WARN_ON_ONCE(!rcu_read_lock_held());

	do {
		/* lru_gen_del_folio() has isolated this folio? */
	...
	} while (!try_cmpxchg(&folio_flags(folio), &old_flags, new_flags));
...
static int folio_inc_gen(struct lruvec *lruvec, struct folio *folio, bool reclaiming)
{
	...
	do {
		...
		/* folio_update_gen() has promoted this folio? */
			       ...
			       bool force_scan)
{
	 ...
	 */
	if (!force_scan &&
	    !(arch_has_hw_pte_young() && get_cap(LRU_GEN_MM_WALK))) {
...
static unsigned long get_nr_evictable(struct lruvec *lruvec, unsigned long max_seq,
				      unsigned long *min_seq, bool can_swap,
				      bool *need_aging)
{
	int gen, type, zone;
	unsigned long old = 0;
...
#if defined(CONFIG_TRANSPARENT_HUGEPAGE) || defined(CONFIG_ARCH_HAS_NONLEAF_PMD_YOUNG)
static void walk_pmd_range_locked(pud_t *pud, unsigned long next,
...
static bool sort_folio(struct lruvec *lruvec, struct folio *folio, int tier_idx)
{
	...
	if (gen != lru_gen_from_seq(lrugen->min_seq[type])) {
		list_move(&folio_lru(folio), &lrugen->lists[gen][type][zone]);
		return true;
	}
	...
	if (tier > tier_idx) {
		...
		list_move_tail(&folio_lru(folio), &lrugen->lists[gen][type][zone]);
			  ...
			  struct scan_control *sc)
{
	...
	if (!folio_test_referenced(folio))
		set_mask_bits(&folio_flags(folio), LRU_REFS_MASK | LRU_REFS_FLAGS, 0);
	...
	for (zone = sc->reclaim_idx; zone >= 0; zone--) {
		...
		while (!list_empty(head)) {
			...
			if (sort_folio(lruvec, folio, tier))
				...
				list_add(&folio_lru(folio), list);
				isolated += delta;
			} else {
				list_move(&folio_lru(folio), &moved);
				skipped += delta;
			}
			...
			int swappiness)
{
	...
	list_for_each_entry(folio, &list, page.lru) {
	...
	if (current->reclaim_state && current->reclaim_state->mm_walk &&
	    current->reclaim_state->mm_walk->batched)
		reset_batch_size(lruvec, current->reclaim_state->mm_walk);
...
void lru_gen_init_lruvec(struct lruvec *lruvec)
{
	...
	for (i = 0; i < NR_HIST_GENS; i++)
		atomic_long_set(&lrugen->evicted[i], 0);

	for (type = 0; type < ANON_AND_FILE; type++) {
		for (i = 0; i < NR_HIST_GENS; i++)
			atomic_long_set(&lrugen->refaulted[i][type], 0);
		lrugen->min_seq[type] = 0;
	}
...
static int lru_gen_seq_show(struct seq_file *m, void *v)
{
	...
	for (; seq <= max_seq; seq++) {
		...
		for (type = 0; type < ANON_AND_FILE; type++) {
			...
			for (zone = 0; zone < MAX_NR_ZONES; zone++)
				size += max(READ_ONCE(lrugen->nr_pages[gen][type][zone]),
					    0L);
			    ...
			    bool reclaimed, struct mem_cgroup *target_memcg)
{
	...
	if (PageSwapCache(page)) {
		...
		/* get a shadow entry before mem_cgroup_swapout() clears page memcg */
